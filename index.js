...
function matchByName(items, name) {
  ...
  return (
    items.find((i) => i.url && (i.url === name || name.endsWith(i.url))) || null
  );
}
  ...
  async updateTask(uid, calendarName, updates) {
    ...
    if (updates.dueDate) {
      ...
      vtodo = this._updateProperty(
        vtodo,
        "DUE",
        format(due, "yyyyMMdd'T'HHmmss'Z'"),
      );
    }
...
// 6. Talk (Nextcloud Chat/Conversations API)
const Talk = {
  ...
  async listMessages(token, options = {}) {
    ...
    const params = new URLSearchParams({
      lookIntoFuture:
        options.lookIntoFuture !== undefined ? options.lookIntoFuture : 0,
  ...
  try {
    if (command === "notes") {
      if (subCommand === "list") {
        ...
        const content =
          contentIndex !== -1 ? args[contentIndex + 1] : undefined;
        const category =
      ...
      if (subCommand === "list") {
        ...
        const start =
          fromIndex !== -1 ? args[fromIndex + 1] : formatISO(new Date());
        const end =
          toIndex !== -1
            ? args[toIndex + 1]
      ...
      if (subCommand === "list") {
        ...
        const limit =
          limitIndex !== -1 ? parseInt(args[limitIndex + 1], 10) : 50;

        const lookIntoFutureIndex = args.indexOf("--look-into-future");
        const lookIntoFuture =
          lookIntoFutureIndex !== -1
            ? parseInt(args[lookIntoFutureIndex + 1], 10)
            : 0;
        ...
        const replyTo =
          replyToIndex !== -1 ? parseInt(args[replyToIndex + 1], 10) : null;
